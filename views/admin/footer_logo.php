...
<html lang="en">
    ...
    <body>
        ...
        <div class="row-fluid white-card mt20 extra-padding">
            <h4>MANAGE FOOTER LOGO</h4>

            <?php echo $this->session->flashdata('success') ? $this->session->flashdata('success') . "<br>" : ''; ?><br>
            <a href="<?php echo site_url('admin/footer_logo/add'); ?>" class="btn floatRight">ADD NEW FOOTER LOGO</a><br><br>
            <div class="clear"></div>
            <table id="header-navs" class="table no-margin table-striped table-bordered">
                <?php if($footer_logo){?>
                    <?php foreach ($footer_logo as $t) { ?>
                        <tr>
                            <td>
                                <div class="floatLeft leftTitle"><?php echo strtoupper($t->name); ?></td>
                            <td width="35%">
                                <a href="<?php echo site_url('admin/footer_logo/edit/' . $t->id); ?>" class="edit_but btn btn-success">Edit</a>
                                <a href="<?php echo site_url('admin/footer_logo/delete/' . $t->id); ?>" class="del btn btn-danger" onclick="return confirm('Are You sure to delete?');">Delete</a>
                            </td>
                        <tr>
                    <?php }?>
                <?php }else{?>
                    <tr>
                        <td colspan="2">
                            There are no Footer logo added.
                        </td>
                    </tr>
                <?php }?>
            </table>
