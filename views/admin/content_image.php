...
<html lang="en">
    ...
    <body>
        ...
        <div class="row-fluid white-card extra-padding mt20">
            ...
            <span class="floatRight"><a class="btn btn-primary" style="margin-bottom: 10px;" href="<?php echo site_url('admin/content/image_add/'.$id) ?>">Add New</a></span> 
            <?php
            if (!empty($gallery)) {
                ?>
                <table id="header-navs" class="table no-margin table-striped table-bordered">
                    ...
                    foreach ($gallery as $nav) {
                        $ids = $nav->id;
                        $delete_url = site_url('admin/content/delete_image/' . $id.'/'.$ids);
                        ?>
                        <tr>
